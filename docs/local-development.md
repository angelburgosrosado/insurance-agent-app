...
Do not read or commit `.env*` files. They are ignored by Git. The application currently reads only `DATABASE_PATH` and `SEED_DEMO_DATA`; neither is required for the default local flow.
...
`pnpm test` runs `tests/db.test.mjs` with Node's test runner and `tsx`. The tests use `:memory:` SQLite databases and cover lead creation, status/follow-up updates, and internal notes. `pnpm start` starts the previously generated production build:
...
The admin endpoints have no authentication or authorization. **`/admin` is not production-safe until authentication is complete**, and the same warning applies to `/api/admin/*`. Do not expose these routes to an untrusted network or use them with real prospect data as if they were access-controlled.
...
- There is no migration, backup, hosted database, or production deployment configuration.
